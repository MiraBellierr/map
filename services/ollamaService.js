...
async function generateImageDescription(imgURL) {
    try {
        ...
        const data = await response.json();
        return `Image description: ${data.response}`;
    } catch (error) {
        ...
        try {
            const fallback = await fetch(`${OLLAMA_API_URL}/generate`, {
                ...
                body: JSON.stringify({
                    model: OLLAMA_IMAGE_MODEL,
                    prompt: `Describe this image in detail: ${imgURL}`,
                    stream: false,
                    ...getGPUOptions(),
                }),
            });
...
async function chatBot(guild, query, personality) {
    ...
    console.log("Memory:", remember);
    console.log("GPU Enabled:", OLLAMA_GPU_ENABLED);
...
async function scrapeSearchResults(query) {
    ...
    try {
        ...
        console.error("Error calling Ollama API for search:", error);
...
async function testOllamaConnection() {
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            console.log("Available Ollama models:", data.models.map(m => m.name).join(", "));
        } else {
            console.warn("Could not connect to Ollama API. Make sure Ollama is running at " + OLLAMA_API_URL);
        }
    } catch (error) {
